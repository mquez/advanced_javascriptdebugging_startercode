...
$(document).ready(function() {
	...
	function completePuzzle() {
		$('body').addClass('ai-enabled');
		initGame($tower.eq(0));
		solveHanoi(disksNum, $tower[0], $tower[2])
	}
	...
	function getSpareTower(fromTower, toTower) {
		fromTowerIdx = parseInt(fromTower.id.charAt(fromTower.id.length - 1), 10);
		toTowerIdx = parseInt(toTower.id.charAt(toTower.id.length - 1), 10);
    	if(fromTowerIdx === null || toTowerIdx === null){ return null; }
    	var spareTowerIdx = 3 - (fromTowerIdx - 1) - (toTowerIdx - 1);
    	return $tower[spareTowerIdx];
	}

	function solveHanoi(numDisks, fromTower, toTower) {
		if(numDisks === 1) {
        	moveDisk(fromTower, toTower);
    	} else {
        	var spareTower = getSpareTower(fromTower, toTower);
        	solveHanoi(numDisks - 1, fromTower, spareTower);
        	moveDisk(fromTower, toTower);
        	solveHanoi(numDisks - 1, spareTower, toTower);		
		}
	}

	// Init Game
	function initGame(tower) {
		$('body').removeClass('ai-enabled');
		$tower.html('');
		moves = 0;
		$movesCount.html(0);
		holding = [];
	...
	function countMove() {
		...
		if (moves > minMoves - 1) {
			if ($tower.eq(1).children().length === disksNum || $tower.eq(2).children().length === disksNum) {
				if($('body').containsClass('ai-enabled')) {
					return;
				}
				swal({
